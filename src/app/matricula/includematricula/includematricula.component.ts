...
import { modelStatus, modelUnit,  User, Revision, Property, Client, Unit} from "./../../core/models";
...
@Component({
  selector: 'app-includematricula',
  templateUrl: './includematricula.component.html',
  styleUrls: ['./includematricula.component.scss']
})

export class IncludematriculaComponent implements OnInit {
  ...
  step_img = require("./../../../assets/layout/images/audsat-images/cadastro/matriculas/5etapas/0-5r.svg");
  ...
  goTo(i:number){
    ...
    this.step_img = require("./../../../assets/layout/images/audsat-images/cadastro/matriculas/5etapas/"+i+"-5r.svg");
    if(i==1){
  ...
  save() {
    ...
    // this.registryStep.guarantee.operationRevision
    ...
    this.matriculaService.saveWizard(
      ...
      this.propertyStep.guarantee.revision,
      this.registryStep.guarantee.revision
    ).then((revision:any) => {
      // TODO: hide loading
      this.hideDialog();
    }).catch(error => {
    ...
    this.messageService.add({ severity:'sucess', summary:'Include com sucesso', detail:'A matricula foi incluida'});
  }
